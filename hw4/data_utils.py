...
def _sample_farthest_point_indices(points, num_samples):
    """Return deterministic farthest-point samples using pure PyTorch."""
    num_points = points.shape[0]
    num_samples = min(num_samples, num_points)
    if num_samples == num_points:
        return torch.arange(num_points)

    indices = torch.empty(num_samples, dtype=torch.long)
    min_squared_distances = torch.full((num_points,), torch.inf)

    # A point far from the centroid gives deterministic spatial coverage from
    # the first iteration onward.
    ...
    with torch.no_grad():
        for sample_idx in range(num_samples):
            ...
            squared_distances = torch.sum(
                (points - points[farthest]) ** 2, dim=1
            )
            min_squared_distances = torch.minimum(
                min_squared_distances, squared_distances
            )
            farthest = min_squared_distances.argmax()
    return indices
...
def read_images_text(path):
    ...
    sorted_images = dict(sorted(
        images.items(), key=lambda item: _natural_sort_key(item[1]['name'])
    ))
...
class ColmapDataset(Dataset):
    def __init__(self, data_path, downsample_factor=8, maximum_pts_num = 3000):
        ...
        # Bound the quadratic rasterization cost while preserving spatial
        # coverage of the sparse reconstruction.
        if maximum_pts_num and len(self.points3D_xyz) > maximum_pts_num:
            indices = _sample_farthest_point_indices(
                self.points3D_xyz, maximum_pts_num
            )
