...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        if self.n_points < 2:
            mean_dists = torch.ones((self.n_points, 1), dtype=points.dtype)
        else:
            # Compute the K nearest-neighbor distances in chunks so scale
            # initialization stays pure PyTorch without allocating the full
            # N x N distance matrix at once.
            ...
            with torch.no_grad():
                for start in range(0, self.n_points, chunk_size):
                    ...
                    dists = torch.cdist(points[start:end], points)
                    rows = torch.arange(end - start)
                    cols = torch.arange(start, end)
                    dists[rows, cols] = torch.inf
                    nearest = torch.topk(
                        dists, k=K, dim=1, largest=False, sorted=False
                    ).values
                    nearest_distances.append(nearest.mean(dim=1, keepdim=True))
            mean_dists = torch.cat(nearest_distances, dim=0) * 2.0

        # Use log space for unconstrained optimization.  The median floor also
        # handles duplicate input points without producing log(0).
        median_distance = torch.median(mean_dists).clamp_min(1e-6)
        mean_dists = mean_dists.clamp(
            0.2 * median_distance, 3.0 * median_distance
        )
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        # Paper Eq. (6): Sigma = R S S^T R^T.  Constructing the
        # intermediate transform first also guarantees a symmetric,
        # positive semi-definite covariance matrix.
        transform = torch.bmm(R, S)
        Covs3d = torch.bmm(transform, transform.transpose(1, 2))
