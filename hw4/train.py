...
class TrainConfig:
    ...
    maximum_pts_num: int = 3000
    foreground_weight: float = 0.0
    edge_weight: float = 0.0
    model_prefix: str = "checkpoint"
...
class GaussianTrainer:
    ...
    def save_checkpoint(self, epoch: int):
        ...
        path = (
            ...
            / f"{self.config.model_prefix}_{epoch:06d}.pt"
        )
    ...
    def train_step(self, batch: dict, in_train = True) -> float:
        ...
        # Compute RGB reconstruction loss.  Synthetic scenes use a black
        # background, so optional foreground weighting prevents empty pixels
        # from dominating the optimization target.
        pixel_l1 = torch.abs(rendered_images - images).mean(dim=-1)
        if self.config.foreground_weight > 0:
            foreground = images.amax(dim=-1) > 0.02
            pixel_weights = 1.0 + self.config.foreground_weight * foreground
            loss = (pixel_l1 * pixel_weights).sum() / pixel_weights.sum()
        else:
            loss = pixel_l1.mean()

        # Matching finite differences penalizes blurred object boundaries and
        # high-frequency texture without adding another model dependency.
        if self.config.edge_weight > 0:
            rendered_dx = rendered_images[:, :, 1:] - rendered_images[:, :, :-1]
            target_dx = images[:, :, 1:] - images[:, :, :-1]
            rendered_dy = rendered_images[:, 1:] - rendered_images[:, :-1]
            target_dy = images[:, 1:] - images[:, :-1]
            edge_loss = (
                torch.abs(rendered_dx - target_dx).mean()
                + torch.abs(rendered_dy - target_dy).mean()
            )
            loss = loss + self.config.edge_weight * edge_loss
    ...
    def train(self, train_loader: DataLoader, start_epoch: int = 0):
        ...
        # save_every may not divide the requested epoch count.  Always retain
        # the final trained state so rendering does not fall back to an older
        # periodic checkpoint.
        final_epoch = self.config.num_epochs - 1
        if final_epoch >= start_epoch and final_epoch % self.config.save_every != 0:
...
def parse_args():
    ...
    parser.add_argument('--model_prefix', type=str, default='checkpoint',
                      help='Prefix used for saved model files')
    parser.add_argument('--debug_dir_name', type=str, default='debug_images',
                      help='Name of the debug image subdirectory')
    parser.add_argument('--video_name', type=str, default='debug_rendering.mp4',
                      help='Name of the comparison video')
    ...
    parser.add_argument('--downsample_factor', type=int, default=8,
                      help='Image downsampling factor used for training')
    parser.add_argument('--maximum_pts_num', type=int, default=3000,
                      help='Maximum number of sampled COLMAP points')
    parser.add_argument('--foreground_weight', type=float, default=0.0,
                      help='Additional L1 weight for non-black foreground pixels')
    parser.add_argument('--edge_weight', type=float, default=0.0,
                      help='Weight of the image-gradient reconstruction loss')
...
def main():
    ...
    config = TrainConfig(
        ...
        log_dir=os.path.join(args.checkpoint_dir, args.debug_dir_name),
    ...
    dataset = ColmapDataset(
        ...
        maximum_pts_num=config.maximum_pts_num,
    )
    ...
    print(
        f"Training on {len(dataset)} images from epoch {start_epoch} "
        f"through {config.num_epochs - 1}"
    )
    ...
    trainer.visualize_rendering(
        dataset, os.path.join(args.checkpoint_dir, args.video_name)
    )
