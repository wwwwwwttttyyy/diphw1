...
def create_mask_from_points(points, img_h, img_w):
    ...
    polygon_points = [tuple(map(int, point)) for point in points]
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    foreground_laplacian = torch.nn.functional.conv2d(
        foreground_img, laplacian_kernel, padding=1, groups=channels
    )
    blended_laplacian = torch.nn.functional.conv2d(
        blended_img, laplacian_kernel, padding=1, groups=channels
    )

    foreground_mask_expanded = foreground_mask.bool().expand(-1, channels, -1, -1)
    background_mask_expanded = background_mask.bool().expand(-1, channels, -1, -1)
